    ...
    <section id="menu" className="relative py-28 md:py-36 bg-[oklch(0.96_0.015_70)]">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center max-w-2xl mx-auto">
          <Reveal><div className="inline-flex items-center gap-2 rounded-full bg-[var(--chili)]/10 text-[var(--chili)] px-3 py-1 text-xs uppercase tracking-[0.25em]">{t.tag}</div></Reveal>
            ...
            <div key={cat.id}>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {cat.items.map((item, i) => (
                  <Reveal key={item.name} delay={i * 40}><Card item={item} t={t} /></Reveal>
                ))}
              </div>
            </div>
          ))}
