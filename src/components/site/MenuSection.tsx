...
function Card({ item, featured = false }: { item: MenuItem; featured?: boolean }) {
  ...
  return (
    <div className={`group relative rounded-2xl border bg-card overflow-hidden hover-lift ${featured ? "p-6 ring-2 ring-[var(--chili)]/20" : "p-5"}`}>
      ...
      {featured && (
        <div className="absolute top-0 inset-x-0 h-0.5 bg-gradient-fire" />
      )}
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <h3 className={`font-display font-semibold leading-snug ${featured ? "text-2xl" : "text-xl"}`}>
            {item.name}
          </h3>
        </div>
        <span className={`font-display text-[var(--chili)] tabular-nums shrink-0 ${featured ? "text-3xl" : "text-2xl"}`}>
          ${item.price}
        </span>
      </div>

      {item.description && (
        <p className={`mt-2 text-muted-foreground leading-relaxed line-clamp-2 ${featured ? "text-base" : "text-sm"}`}>
          {item.description}
        </p>
      )}
      ...
      >
        <div className="overflow-hidden">
          ...
          {item.note && (
            <p className="mt-3 text-xs text-[var(--chili)]/80 italic">{item.note}</p>
          )}
        </div>
      </div>
    </div>
...
export function MenuSection() {
  ...
  const cats = useMemo(
    () => [{ id: "all", title: "All" }, ...MENU.map((c) => ({ id: c.id, title: c.title }))],
    []
  );
            ...
            <Reveal>
              <div className="flex items-center gap-4 mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-display text-2xl md:text-3xl">Most Loved</h3>
                </div>
                <div className="h-px flex-1 bg-gradient-to-r from-[var(--chili)]/30 to-transparent" />
                <span className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Guest favorites</span>
              </div>
            </Reveal>
              ...
              <Reveal>
                <div className="flex items-end justify-between flex-wrap gap-3 mb-6">
                  <div>
                    ...
                    {cat.blurb && (
                      <p className="text-muted-foreground mt-1">{cat.blurb}</p>
                    )}
                  </div>
